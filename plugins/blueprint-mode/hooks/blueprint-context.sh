...
if ! echo "$PROMPT" | grep -qE '/blueprint:'; then
  if echo "$PROMPT" | grep -qiE '\bblueprint\b|\badrs?\b|architecture.decision|docs/specs|docs/adrs|patterns/(good|bad)|boundaries\.md|tech-stack\.md|feature.spec|anti-pattern|design/ux-decisions|design/patterns|ux.decision|design.tree|design.pattern'; then
