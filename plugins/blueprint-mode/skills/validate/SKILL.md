...
**Vocabulary.** A legacy `## Always Do` heading in boundaries is read as `## Safe Without Asking`; obligation-style bullets under it ("read X before Y", "run Z before commit") are Low with the note that they belong in the agent instructions, not boundaries. Non-canonical synonyms are Low: in decisions `Benefits`, `Trade-offs`, `Pros`, `Cons`, `References`, `status: Accepted`, and an `# ADR-` title under `design/ux-decisions/`; in feature specs `## Description`, `## Stories`, `status: Done|Complete|Todo`; in boundaries `# Boundaries`, `Do Always`, `Ask Before`, `Don't Do`, `Prohibited`; in anti-patterns `Wrong Way`, `Right Way`, `Better`, `**Level:**`.
