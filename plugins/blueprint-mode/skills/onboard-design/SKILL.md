...
6. Update the agent instructions file per `agent-file-detection`. Add the `DESIGN.md` and `design/ux-decisions/` lines from the `agent-instructions` template to its "Where intent lives" list. If the file has no such list, it predates 2.0: regenerate the Blueprint section as `/blueprint-mode:onboard` does, then add the lines.
