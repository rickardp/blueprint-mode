...
- Capture deliberate UX choices as `design/ux-decisions/NNN-*.md` records when the design tree exists. Authoring is conversational (via `/blueprint-mode:onboard-design` optional triage or `/blueprint-mode:decide`), never a hand-filled template.
...
- `/blueprint-mode:onboard-design` may optionally run an existing-UI triage that scans the UI/code for a small number (≤5) of patterns and uses them as prompts: it asks the user, developer, or designer whether each was a conscious choice and what the rationale was, then captures only those with articulated intent. Code is the prompt, not the source of truth — the human supplies the why.
...
- `/blueprint-mode:validate` checks UI code against Active UX decisions and surfaces `// UX-TBD:` flag counts for review.
...
| `/blueprint-mode:onboard-design` offers optional intent-capture triage (scan UI → user/developer/designer articulates which were conscious choices and why → capture those) | Done |
...
| `/blueprint-mode:validate` checks UX decision compliance and reports UX-TBD flag inventory | Done |
| `DESIGN.md` treated as community-owned top-level design context outside the Blueprint structure — `/blueprint-mode:onboard-design` offers to scaffold a compatible stub and wires it into agent instructions | Done |
| `DESIGN.md` vs `design/ux-decisions/` delineation documented in TEMPLATES.md and help skill | Done |
| `/blueprint-mode:decide`, `/blueprint-mode:capture`, `/blueprint-mode:status`, and `/blueprint-mode:validate` treat `DESIGN.md` as a first-class repo artifact | Done |
...
- Do not require designers to hand-fill markdown templates. Authoring is conversational via `/blueprint-mode:onboard-design` triage or `/blueprint-mode:decide`.
...
- Capture intent, not inferred structure. UX decisions exist for *conscious* choices with a stated rationale; current code shape on its own is not evidence a choice was deliberate. Existing-UI triage is optional and caps candidates at ~5 to keep the conversation focused on choices a human can articulate.
