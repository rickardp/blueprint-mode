...
- Capture deliberate UX choices as `design/ux-decisions/UX-NNN-*.md` records when the design tree exists. Authoring is conversational (via `/blueprint:onboard-design` optional triage or `/blueprint:decide`), never a hand-filled template.
...
| `/blueprint:onboard-design` offers optional intent-capture triage (scan UI → user/developer/designer articulates which were conscious choices and why → capture those) | Done |
